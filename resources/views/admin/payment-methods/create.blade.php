...
@section('header')
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h1 class="h3 mb-0 fw-bold text-gray-800">Create Payment Method</h1>
            <p class="text-muted mb-0">Add a new payment method for checkout</p>
        </div>
        <div>
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('admin.payment-methods.store') }}">
                            ...
                            <div class="mb-3">
                                ...
                                <input id="type" type="text" name="type" class="form-control" value="{{ old('type') }}" list="pm-types" placeholder="e.g., COD" required>
                                ...
                                <div class="form-text">Examples: COD, Bank Transfer, Mobile Wallet, Online Payment.</div>
                                <x-input-error :messages="$errors->get('type')" />
                            </div>

                            <div class="mb-3">
                                <x-input-label for="name" value="Payment Method Name" />
                                <x-text-input id="name" type="text" name="name" value="{{ old('name') }}" placeholder="e.g., KBZ Pay" required />
                                <x-input-error :messages="$errors->get('name')" />
                            </div>

                            <div class="mb-3">
                                <x-input-label for="account_number" value="Account / Number" />
                                <x-text-input id="account_number" type="text" name="account_number" value="{{ old('account_number') }}" placeholder="e.g., 09xxxxxxxxx / Bank Account" required />
                                <x-input-error :messages="$errors->get('account_number')" />
                                <div class="form-text">Use "N/A" for COD if needed.</div>
                            </div>
